...
function ical_feed()
{
    ...
    //$niceeventtime = date('Y-m-d H:i:s', $eventtime[0]);
		$start_time = date('Ymd\THis', $eventtime[0]);
    
    /*
		if ($endeventtime[0]) { 
		  $end_time = date('Ymd\THis', $endeventtime[0]);		
    }
    */
...
EVENT;
	...
	if (!defined('DEBUG'))
	{
	  ...
	  header('Content-Disposition: attachment; filename="pbpav_events.ics"');
	}
